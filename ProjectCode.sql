...
#Correlated subquery that lists each club member as well as their highest transaction amount.
#This query is relevant because it provides valuable insight into the spending behavior of members.
#The manager is able to monitor member engagement, optimize revenue, and identify high and low spending members.
SELECT
memberID,
memberFirstName,
memberLastName,
...
SELECT SUM(PSI.productPrice) AS TotalRevenue
FROM ProShopItem AS PSI
JOIN Receipt AS R ON PSI.productID = R.ProShopItem_productID;


#Keeps track of tournament participation between members and the total amount they have paid in registering.
